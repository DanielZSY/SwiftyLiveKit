...
/// 滚动文本
public class ZAutoScrollLabel: UIView {
    ...
    public var scrollSpeed = ZAutoScrollLabel.kDefaultPixelsPerSecond {
        didSet {
            scrollLabelIfNeeded()
        }
    }
    ...
    // Defaults to 1.5
    public var pauseInterval = ZAutoScrollLabel.kDefaultPauseTime
    
    // Pixels, defaults to 20
    public var labelSpacing = ZAutoScrollLabel.kDefaultLabelBufferSpace
    ...
    // Defaults to 7
    public var fadeLength = ZAutoScrollLabel.kDefaultFadeLength {
        didSet {
    ...
    //        get {
    //            return self.mainLabel.shadowColor ?? UIColor.clear
    //        }
    //        set {
    //            for lab in labels {
    ...
    private var labels: [UILabel] = {
        var ls = [UILabel]()
        for index in 0 ..< ZAutoScrollLabel.kLabelCount {
            ls.append(UILabel())
        }
        return ls
    }()
    ...
    private func commonInit() {
        assert(ZAutoScrollLabel.kLabelCount > 0, "ZAutoScrollLabel.kLabelCount must be greater than zero!")
        ...
        self.scrollSpeed = ZAutoScrollLabel.kDefaultPixelsPerSecond
        self.pauseInterval = ZAutoScrollLabel.kDefaultPauseTime
        self.labelSpacing = ZAutoScrollLabel.kDefaultLabelBufferSpace
        self.fadeLength = ZAutoScrollLabel.kDefaultFadeLength
    ...
    public func observeApplicationNotifications() {
        ...
        NotificationCenter.default.addObserver(
            self, selector: #selector(ZAutoScrollLabel.scrollLabelIfNeeded),
    ...
    func scrollLabelIfNeededAction() {
        ...
        NSObject.cancelPreviousPerformRequests(
            withTarget: self, selector: #selector(ZAutoScrollLabel.scrollLabelIfNeeded), object: nil
        )
        NSObject.cancelPreviousPerformRequests(
            withTarget: self, selector: #selector(ZAutoScrollLabel.enableShadow), object: nil
        )
        
        self.scrollView.layer.removeAllAnimations()
        
        let doScrollLeft = self.scrollDirection == ZAutoScrollDirection.Left
        self.scrollView.contentOffset = doScrollLeft ? .zero : CGPoint(
            x: labelWidth + self.labelSpacing, y: 0
        )
        
        self.perform(
            #selector(ZAutoScrollLabel.enableShadow), with: nil, afterDelay: self.pauseInterval
        )
            ...
            if let strongSelf = self {
                ...
                if finished {
                    strongSelf.performSelector(
                        inBackground: #selector(ZAutoScrollLabel.scrollLabelIfNeeded), with: nil
                    )
                }
            }
        }
    ...
    private func onUIApplicationDidChangeStatusBarOrientationNotification(notification: NSNotification) {
        ...
        perform(#selector(ZAutoScrollLabel.refreshLabels), with: nil, afterDelay: 0.1)
        perform(#selector(ZAutoScrollLabel.scrollLabelIfNeeded), with: nil, afterDelay: 0.1)
    }
}
