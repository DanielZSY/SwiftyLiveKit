...
class ZRankLoveItemCVC: ZBaseCVC {
    ...
    var z_model: ZModelUserInfo? {
        didSet {
            ...
            guard let userother = user.rank_other_people else { return }
            z_lbusernameother.text = userother.nickname
            z_imagephotoother.setImageWitUrl(strUrl: userother.avatar, placeholder: Asset.anchorAvatar.image)
        }
    }
    ...
    private lazy var z_imagephoto: UIImageView = {
        let z_temp = UIImageView.init(frame: CGRect.init(z_lbnum.width, self.z_viewcontent.height/2 - 43.scale/2, 43.scale, 43.scale))
        z_temp.border(color: .clear, radius: z_temp.height/2, width: 0)
        return z_temp
    }()
    private lazy var z_imagephotoother: UIImageView = {
        let z_temp = UIImageView.init(frame: CGRect.init(z_imagephoto.x + 30.scale, z_imagephoto.y, 43.scale, 43.scale))
        z_temp.border(color: .clear, radius: z_temp.height/2, width: 0)
        return z_temp
    }()
    private lazy var z_lbusername: UILabel = {
        let z_tempx = z_imagephotoother.x + z_imagephotoother.width + 10.scale
        let z_temp = UILabel.init(frame: CGRect.init(z_tempx, z_imagephoto.y + 3.5.scale, z_btnicon.x - z_tempx - 5.scale, 18.scale))
        z_temp.textColor = "#FFFFFF".color
        z_temp.boldSize = 15
        z_temp.adjustsFontSizeToFitWidth = true
        return z_temp
    }()
    private lazy var z_lbusernameother: UILabel = {
        let z_tempx = z_imagephotoother.x + z_imagephotoother.width + 10.scale
        let z_temp = UILabel.init(frame: CGRect.init(z_tempx, z_lbusername.y + z_lbusername.height, z_btnicon.x - z_tempx - 5.scale, 18.scale))
        z_temp.textColor = "#FFFFFF".color
        z_temp.boldSize = 15
        z_temp.adjustsFontSizeToFitWidth = true
        return z_temp
    ...
    override init(frame: CGRect) {
        ...
        z_viewcontent.addSubview(z_lbusernameother)
        z_viewcontent.addSubview(z_imagephoto)
        z_viewcontent.addSubview(z_imagephotoother)
