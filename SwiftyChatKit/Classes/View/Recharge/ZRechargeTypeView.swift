...
class ZRechargeTypeView: UIView {
    ...
    private lazy var z_btntype1: ZRechargeTypeButton = {
        let btnw = (self.width - 15.scale*3)/2
        let z_temp = ZRechargeTypeButton.init(frame: CGRect.init(15.scale, 5.scale, btnw, self.height - 10.scale))
        z_temp.tag = 1
        z_temp.z_isselect = false
        return z_temp
    }()
    private lazy var z_btntype2: ZRechargeTypeButton = {
        let btnw = (self.width - 15.scale*3)/2
        let z_temp = ZRechargeTypeButton.init(frame: CGRect.init(z_btntype1.x + z_btntype1.width + 15.scale, 5.scale, btnw, self.height - 10.scale))
        z_temp.tag = 2
    ...
    @objc private func func_btntypeclick(_ sender: ZRechargeTypeButton) {
